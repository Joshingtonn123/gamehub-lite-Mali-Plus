...
- **Dedicated Mali Optimizations Tab**: In-app controls tailored for Mali GPUs:
  - **DXVK Async Pipeline**: Eliminates shader compilation stuttering on Mali tile-based renderers.
  - **Shader Compiler Threads**: Optimal multithreaded shader compilation for big.LITTLE core architectures.
  - **Tessellation Workaround & Fix**: Bypasses tessellation driver stalls common on Mali drivers.
  - **Box64 BigCore Affinity & Fast Math**: Binds heavy translation tasks to performance Cortex-X and Cortex-A cores.
  - **Panfrost / Mesa Stability Tuning**: Prevents Mali driver crashes and optimizes swapchain presentation modes.
  - **Turnip/Adreno Conflict Prevention**: Automatically clears conflicting Snapdragon drivers to ensure clean Mali execution.
- **Enhanced Frame Generation**:
  - **2x, 3x, and 4x Multipliers**: Choose between balanced (2x), high (3x), or ultra (4x) interpolated frame rates.
  - **Low-FPS Anti-Lag (Anti-Stall)**: Prevents slow-motion and input backlog when base framerate drops below 30 FPS.
- **Multi-Variant OEM Throttling Bypass**: Builds spoofed packages (Antutu, PUBG, Genshin, Geekbench) to trigger OEM maximum clock speed profiles on Dimensity and Mali devices.
- **Lightweight & Privacy-Focused**: Telemetry removed, bloat stripped, offline-ready.
...
**PUBG** and other games spoofing:
    ...
    •    GPU driver optimizations (Mali Immortalis / Valhall game-specific paths)
    ...
    •    MediaTek HyperEngine / Dimensity Gaming Performance Mode
    •    Enables hardware frame generation and display sync
